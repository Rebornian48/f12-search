...
function search(query) {
    ...
    if(query == 'flowerful' || query == 'Flowerful' || query == 'FLOWERFUL'){
        var result = 'Diadakan pada tanggal 17 Desember 2023 di Graha Unesa Surabaya';}
    else if(query == 'vc' || query == 'video call' || query == 'VC' || query == 'videocall'){
        var result = 'Fans JKT48 yang terhormat, kami mohon maaf atas ketidaknyamanannya. Sehubungan dengan masih terdapatnya pembatasan akses masuk ke situs web JKT48, imbas tingginya antusiasme fans JKT48 yang ingin membeli tiket VC. Pembatasan akses masuk ke situs web tercinta akan dilakukan sampai waktu yang belum dapat ditentukan. Terima kasih.';
    }
    else if(query == 'live streaming' || query == 'streaming' || query == 'live stream' || query == 'stream'){
        ...
        var result = 'Maaf, kami tidak dapat menemukan hasil pencarian untuk: ' + query + '. Silahkan coba lagi.';
    }
    searchResults.innerHTML = 'Hasil pencarian untuk: ' + query + "<br />" + result;
}